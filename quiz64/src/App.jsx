...
export default function App() {
  ...
  const commit = (value) => {
    ...
    if (!currentQuestion.test && hadSealedTests && state.answers?.[currentQuestion.id] !== value) {
      const contextChange = currentQuestion.id === 'q01' || currentQuestion.id === 'q1' || currentQuestion.id === 'q02' || currentQuestion.id === 'q2';
      const message = contextChange ? 'Changing this context clears the sealed checks and earlier answers that depend on it. Continue?' : 'Changing this training answer clears the sealed checks so the reading can be rebuilt. Continue?';
    ...
    const invalidated = hadSealedTests && !currentQuestion.test && !next.locked;
  ...
  return <MotionConfig reducedMotion={motionOn ? 'user' : 'always'}>
    <div className="app-shell">
      ...
      <ReviewDialog open={reviewOpen} onClose={() => setReviewOpen(false)} state={state} onVisit={(q) => { if (q.test && !state.answers?.[q.id]) return; const next = { ...state, cursor: questionIndex(q.id) }; setState(next); setScreen('quiz'); }} />
      <MoreDialog open={moreOpen} onClose={() => setMoreOpen(false)} onHow={() => setHowOpen(true)} onReview={() => setReviewOpen(true)} onExport={exportData} onMap={() => setMapOpen(true)} onSave={goHome} onReset={() => setResetOpen(true)} />
