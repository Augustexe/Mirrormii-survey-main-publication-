...
import { DIMS, QUESTIONS, facts, label, interpolate, safeTitle } from '../survey.js';
...
function CheckRow({ trial, state }) {
  ...
  return <div className="check-row"><div><b>{trial.question}</b><span>{status}</span></div><p><strong>Scenario:</strong> {q ? safeTitle(q, state) : trial.question}<br /><strong>Guess:</strong> {interpolate(option(trial.option), state)}<br /><strong>You:</strong> {trial.actual ? interpolate(option(trial.actual), state) : 'Skipped'}</p><small>{trial.reason} · Sources: {trial.sources?.length ? trial.sources.join(', ') : 'none'}</small></div>;
}
