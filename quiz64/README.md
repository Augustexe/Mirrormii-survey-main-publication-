...
The production build uses `base: './'` and resolves local artwork through `import.meta.env.BASE_URL`, so the output can be served from a nested static directory. No runtime network requests, account, analytics, or backend are required; answers are not sent anywhere. The user can export a private JSON attempt at any point.
